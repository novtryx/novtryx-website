...
const Header = () => {
    const navArray = ["Home", "About", "Our Work"]
    ...
    return (
        <div className='py-4 sm:py-5 md:py-6 backdrop-blur-md bg-linear-to-b from-[#A1FFFF]/60 via-[#A1FFFF]/40 to-white flex px-4 sm:px-6 md:px-10 lg:px-12 xl:px-16 justify-between items-center sticky top-0 z-50'>
            ...
            >
                ...
                <p className='text-[20px] sm:text-[24px] md:text-[28px] lg:text-[30px] tracking-tighter font-medium'>NOVTRYX</p>
            </motion.div>
            ...
            >
                {
                    navArray.map((item: string) => (
                        ...
                        >
                            <Link href={"#"} className='font-medium'>{item}</Link>
                        </motion.div>
                    ))
                }
            </motion.div>
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <>
                        ...
                        {/* Menu */}
                        <motion.div
                            className='md:hidden fixed top-[72px] sm:top-[80px] left-0 right-0 bg-white shadow-xl z-50 border-t border-[#008080]/20'
                        ...
                        >
                            <div className='flex flex-col py-4 px-4 sm:px-6'>
                                ...
                                >
                                    <Link 
                                        href="#"
                                        className='flex items-center gap-3 py-3 px-4 bg-[#008080] text-white rounded-lg font-medium'
                                        onClick={() => setIsMobileMenuOpen(false)}
                                    >
